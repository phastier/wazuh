...
- [ ] CoreFilter ([corefilter.app](https://corefilter.app), by Octiplex) log integration — per-application firewall for macOS built on the native Network Extension / Endpoint Security frameworks, with identity-bound policies (Entra ID / Okta) and real-time audit. SIEM forwarding is a native product feature, and we are working **in direct collaboration with CoreFilter's author** so the decoder (per-app, per-user verdict events) is ready alongside the launch (Q2 2026)
